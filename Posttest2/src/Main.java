...
public class Main {
    ...
    static void tambahPesanan(){
        ...
        for(SnackBox s : daftarSnackBox){
            if(s.getNamaPaket().equalsIgnoreCase(paket)){
                harga = s.getHarga();
            }
        }
    ...
    static void updatePesanan(){
        ...
        for(Pesanan p : daftarPesanan){
            ...
            if(p.getIdPesanan().equals(id)){
                ...
                double hargaPerBox = p.getTotalHarga() / p.getJumlah();

                p.setJumlah(jumlahBaru);
                p.setTotalHarga(hargaPerBox * jumlahBaru);
